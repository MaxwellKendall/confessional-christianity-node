/**
 * Domain types for confessional-christianity-node.
 *
 * This is a mostly-JavaScript codebase, so these interfaces exist to document
 * the shape of the core data structures in one place. They are global (this
 * file has no imports/exports), so JS files can reference them from JSDoc
 * (e.g. `@returns {ConfessionEntry}`) and editors will surface them.
 *
 * See docs/DOMAIN.md for the id grammar and the document-alias vocabularies
 * that these shapes are keyed by.
 */

/**
 * A single node in a confession/catechism, as stored in
 * dataMapping/content-by-id.json and returned by loadConfessionContent().
 *
...
interface ConfessionEntry {
  ...
  text?: string;
  /** Proof-text scripture citations, keyed by citation label. */
  verses?: Record<string, string[]>;
}

/** The flat id -> entry map that most components receive as `contentById`. */
type ContentById = Record<string, ConfessionEntry>;

/** Result of loadConfessionContent(slug) in lib/confessionContent.js. */
